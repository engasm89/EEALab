...
function StageChip({ stage }: { stage: string }) {
  ...
  return (
    <span
      className={`inline-flex items-center rounded-full border px-3 py-1 text-[11px] font-semibold ${tone[stage] ?? "border-white/15 bg-white/6 text-muted"}`}
    >
      {stage}
    </span>
  );
            ...
            <div className="max-w-2xl">
              ...
              <div className="mt-7 grid gap-4 sm:grid-cols-2">
                <div className="rounded-2xl border border-white/10 bg-background/30 p-5">
                  ...
                  <div className="mt-1 text-sm leading-relaxed text-muted">{project.problem}</div>
                  <div className="mt-4 text-sm font-semibold">Why it exists</div>
                  <div className="mt-1 text-sm leading-relaxed text-muted">{project.why}</div>
                </div>
                ...
                <div className="rounded-2xl border border-white/10 bg-background/30 p-5">
                  ...
                  <div className="mt-1 text-sm leading-relaxed text-muted">{project.next_step}</div>
          ...
          <div className="mt-10 grid gap-8 lg:grid-cols-2 lg:items-start">
                  ...
                  <div className="mt-4 space-y-3">
                    {(assets.length ? assets : (project.screenshot_urls ?? []).map((url, idx) => ({
                      ...
                      <div key={asset.id} className="overflow-hidden rounded-xl border border-white/10 bg-background/20">
                        ...
                        <div className="px-3 py-2 text-xs text-muted">{asset.caption ?? "Preview"}</div>
                      </div>
                    ))}
            ...
            <div className="space-y-6">
              <div className="rounded-2xl border border-white/10 bg-white/5 p-5">
                ...
                <div className="mt-2 flex flex-wrap gap-2">
                  {project.tech_stack.map((t) => (
                    <span
                      key={t}
                      className="inline-flex items-center rounded-full border border-white/10 bg-background/30 px-3 py-1 text-xs font-semibold text-muted"
                    >
                      {t}
                    </span>
                  ))}
              ...
              <div className="rounded-2xl border border-white/10 bg-white/5 p-5">
                ...
                <div className="mt-2 text-sm text-muted">Chronological updates from problem to shipped iteration.</div>
                <div className="mt-4">
