...
export async function POST(req: Request) {
  ...
  const parsed = UpvoteBodySchema.safeParse(body);
  ...
  const rl = await takeRateLimit(`upvote:${ip}:${fingerprint}`);
  ...
  if (error) {
    // Unique constraint hit: already upvoted by this fingerprint.
